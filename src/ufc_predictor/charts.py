...
from ufc_predictor.config import DEFAULT_PATHS, ProjectPaths
...
def generate_charts(paths: ProjectPaths = DEFAULT_PATHS) -> None:
    paths.chart_dir.mkdir(parents=True, exist_ok=True)
    metrics = pd.read_csv(paths.metrics_output)
    calibration = pd.read_csv(paths.calibration_output)
    confidence = pd.read_csv(paths.confidence_output)

    write_svg(paths.chart_dir / "model_comparison.svg", bar_chart(metrics))
    write_svg(paths.chart_dir / "calibration_buckets.svg", calibration_chart(calibration))
    write_svg(paths.chart_dir / "confidence_buckets.svg", confidence_chart(confidence))


def main() -> None:
    generate_charts()
    print("Saved charts")
    print(f"  {DEFAULT_PATHS.chart_dir / 'model_comparison.svg'}")
    print(f"  {DEFAULT_PATHS.chart_dir / 'calibration_buckets.svg'}")
    print(f"  {DEFAULT_PATHS.chart_dir / 'confidence_buckets.svg'}")
