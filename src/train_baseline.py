...
def main() -> None:
    ...
    y_train = train["fighter_a_wins"].to_numpy(dtype=float)
    y_test = test["fighter_a_wins"].to_numpy(dtype=float)

    x_train, x_test, feature_names, means, stds = prepare_design_matrix(train, test)
    weights = train_logistic_regression(x_train, y_train)
    test_prob = sigmoid(x_test @ weights)
    train_prob = sigmoid(x_train @ weights)

    majority_prob = np.repeat(y_train.mean(), len(y_test))
    metrics = [
        {
